...
function App() {
  ...
  const showingPosts = showPosts.map((post) => <Grid item key={post.id}><AdCard adTitle={post.name} adText={post.text} key={post.id} post_id={post.id}></AdCard></Grid>); // id обязательно надо для reacta
  return (
